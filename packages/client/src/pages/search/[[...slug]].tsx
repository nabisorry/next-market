...
const categoryNameDict: Record<ProductCategory, string> = {
  book: '책',
  shoes: '신발',
  clothes: '의류',
};
...
const SearchPage: NextPage = () => {
  ...
  // 상품 카테고리를 쿼리로부터 얻는다
  const slug: ProductCategory[] = Array.isArray(router.query.slug)
    ? (router.query.slug as ProductCategory[])
    : [];
  // 상품 상태를 쿼리로부터 얻는다
  const conditions = (() => {
    if (Array.isArray(router.query.condition)) {
      return router.query.condition as ProductCondition[];
                  ...
                  (category: string, i: number) => (
                    <Box key={i} marginTop={1}>
                      <Link href={`/search/${category}`} passHref>
                        <Anchor>
                          {categoryNameDict[category as ProductCategory]}
                        </Anchor>
                      </Link>
                    </Box>
                  ),
